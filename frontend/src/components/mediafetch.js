...
function YoutubeEmbed() {
  ...
  function Counter() {
    return (
      <div>
        <Button onClick={() => setCount(count - 1)}>Play Prev</Button>
        <Button onClick={() => setCount(count + 1)}>Play Next</Button>
        <p style={{ color: "tomato" }}>Playing : {data ? data.title : null} </p>
        <hr />
        {videoData
          ? videoData.map((item, i) => (
              <li key={item + i} onClick={() => setVidUrl(item.videoUrl)}>
                {item.title}
              </li>
            ))
          : null}
      ...
      >
        <Card style={{ width: "42rem" }}>
          ...
          <ListGroup className="list-group-flush">
            <ListGroupItem
              style={{ marginLeft: "2%", backgroundColor: "#e6d1da" }}
            >
              <Card.Img
                variant="top"
                style={{
              ...
              {videoData
                ? videoData.map((x) =>
                    x.videoUrl == vidUrl ? <p>{x.postedBy}</p> : null
                  )
                : null}
            ...
            <ListGroupItem>
              {data ? <p>Description: {data.desc}</p> : null}
            </ListGroupItem>
          </ListGroup>
          <Card.Body>
            <h3>Production note:</h3>
            <p style={{ color: "red" }}>Viewers' comments will appear here</p>
          </Card.Body>
        </Card>
        <div>
          <Card
